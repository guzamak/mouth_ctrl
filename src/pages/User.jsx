    ...
    if (!finduser.docs[0]) {
      ...
      if (querySnapshot) {
        const allimg = await Promise.all(
          querySnapshot.docs.map(async (img) => {
            const url = await getDownloadURL(ref(storage, img.id));
            const imgdata = img.data();
            imgdata.owner = (
              await getDoc(doc(db, "user", imgdata.userId))
            ).data().username;
            imgdata.url = url;
  ...
  const like = async (imgId) => {
    ...
    if (user) {
      ...
      if (userLike.includes(user.uid)) {
        ...
        newImgList[imgIndex].data.userLike = userLike.filter(
          (uid) => uid !== user.uid
        );
      ...
      if (ordertype == "Recently") {
        ...
        querySnapshot = await getDocs(
          query(
            ...
            orderBy("timestamp", "desc"),
            startAfter(
              imgList[imgList.length - 1].data.like,
              imgList[imgList.length - 1].data.timestamp
            ),
            limit(10)
          )
        );
            ...
            <>
              ...
              <div className="hero min-h-[50vh] bg-gray-100 shadow-inner">
                <div className="flex flex-col justify-center items-start px-[5vw] w-full h-full">
                  ...
                  <p className=" my-3 font-extralight">user/{username}</p>
                </div>
              </div>
                ...
                <div className="container py-10 mx-auto">
                  <div className="flex w-full max-w-[30rem] ">
                    <button
                      ...
                      onClick={() => {
                        setOderType("Recently");
                      }}
                    >
                      Recently
                    </button>
                    <button
                      className={`type-btn mx-[8%] ${
                        ordertype === "Popular" ? "text-black" : "text-gray-300"
                      }`}
                      onClick={() => {
                        setOderType("Popular");
                      }}
                    ...
                    {imgList.length != 0 &&
                      imgList.map((img) => {
                        return (
                          <div
                            key={img.id}
                            className="h-96 overflow-hidden rounded-2xl shadow-[rgba(50,_50,_105,_0.10)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.025)_0px_1px_1px_0px]"
                          >
                            {/* img */}
                            ...
                            <div className="flex px-5 py-5 w-full justify-between items-center ">
                              ...
                              <div className="flex justify-center items-center ">
                                ...
                                >
                                  {user ? (
                                    img.data.userLike.includes(user.uid) ? (
                                      <AiFillHeart />
                                    ) : (
                                      <AiOutlineHeart />
                                    )
              ...
              {!imgLoading ? (
                <div className="flex justify-center my-9">
                  <button
                    className="border border-gray-500 text-gray-500 text-center p-2 px-5 rounded-xl hover:bg-black hover:text-white hover:border-none transition-all duration-200 ease-in"
                    onClick={getMoreData}
                  >
                    Load more img
                  </button>
